import logging
import os\n\ntry:\n    from backend.common.configuration import ConfigNotFoundError, get_hub\nexcept ImportError:\n    ConfigNotFoundError = None  # type: ignore\n    get_hub = None  # type: ignore
...
def _should_enable_mock(mock_flag: bool) -> bool:
    ...
    if get_hub is None:
        return False
    try:
        return bool(get_hub().get("benchmark.use_mock_services"))
    except (AttributeError, ConfigNotFoundError):
        return False
    except Exception:
        logger.debug("Failed to read benchmark.use_mock_services", exc_info=True)
        return False
