...
class AgentProtocolServer:
    ...
    async def execute_step(self, task_id: str, step_request: StepRequestBody) -> Step:
        ...
        if step_request.input:
            agent.persist_working_context(
                f"Step {step.step_id} input: {step_request.input}",
                metadata={"task_id": task_id, "step_id": step.step_id, "type": "input"},
            )
        ...
        if execute_command:
            ...
            if execute_command == ask_user.__name__:  # HACK
                ...
                try:
                    ...
                    agent.persist_working_context(
                        f"Step {step.step_id} result: {execute_result}",
                        metadata={"task_id": task_id, "step_id": step.step_id, "type": "result"},
                    )
                    agent.archive_long_term_summary(
                        f"{execute_command} -> {execute_result}",
                        metadata={"task_id": task_id, "step_id": step.step_id, "category": "action_result"},
                    )
        ...
        memory_query = user_input or step_request.input or task.input
        memory_context = (
            agent.search_recent_context(memory_query, limit=3) if memory_query else {"working": [], "long_term": []}
        )
