...
class FileContextItem(BaseModel, ContextItem):
    ...
    workspace_path: Path
    workspace: Optional[FileStorage] = None
    ...
    def file_path(self) -> Path:
        if self.workspace:
            return self.workspace.get_path(self.file_path_in_workspace)
    ...
    def content(self) -> str:
        if self.workspace:
            file_obj = self.workspace.open_file(self.file_path_in_workspace, binary=True)
            context_manager = closing(file_obj)
        else:
            context_manager = open(self.file_path, "rb")
        extension = os.path.splitext(self.file_path.name)[1]
        with context_manager as file:
            return decode_textual_file(file, extension, logger)
