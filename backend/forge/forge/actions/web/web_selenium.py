...
async def read_webpage(
    ...
    return await asyncio.to_thread(_read_webpage_impl, url, question)


def _read_webpage_impl(url: str, question: str = "") -> Tuple(str, List[str]):
    driver = None
    try:
...
async def fetch_webpage_async(
    url: str,
    question: str = "",
    *,
    timeout: float | None = 30.0,
) -> Tuple(str, List[str]):
    """Utility for fetching a webpage outside the command framework."""

    try:
        return await asyncio.wait_for(
            asyncio.to_thread(_read_webpage_impl, url, question),
            timeout=timeout,
        )
    except asyncio.TimeoutError as exc:
        raise BrowsingError("Timed out while fetching webpage") from exc
