...
def build_memory_items(n_items: int, n_chunks: int, dim: int) -> list[MemoryItem]:
    ...
    for _ in range(n_items):
        ...
        weights = np.random.randint(1, 100, size=n_chunks)
...
def main() -> None:
    ...
    results = {
        "summary": {"recall": acc_sum, "latency": t_sum},
        "weighted": {"recall": acc_wgt, "latency": t_wgt},
    }
    print("Embedding strategy benchmark:")
    for strat, metrics in results.items():
        print(f"{strat:<8} recall: {metrics['recall']:.2%}, latency: {metrics['latency']:.4f}s")

    results_path = Path(__file__).resolve().parent / "results" / "memory_embedding_strategy.json"
