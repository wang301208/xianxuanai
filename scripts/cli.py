...
# Ensure repo root is on sys.path so imports like `third_party.*` work when running this file directly.
_repo_root = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
if _repo_root not in sys.path:
    sys.path.insert(0, _repo_root)

# Some Windows consoles use legacy encodings (e.g. gbk) and crash on emoji output.
...
from third_party.autogpt.autogpt.core.logging import handle_exception, setup_exception_hooks
...
@cli.command()
@click.option("--check-only", is_flag=True, help="Only verify prerequisites (no installs)")
@click.option("--skip-system-deps", is_flag=True, help="Skip checking/installing system tools")
@click.option("--skip-deps", is_flag=True, help="Skip installing project dependencies")
@click.option("--skip-assets", is_flag=True, help="Skip checking/downloading model/data assets")
@click.option("--fetch-assets", is_flag=True, help="Download missing model/data assets after setup")
@click.option(
    "--assets-manifest",
    default=None,
    help="Path to assets manifest (default: config/assets.json)",
)
@click.option("--yes", is_flag=True, help="Assume 'yes' when downloading assets")
def setup(check_only, skip_system_deps, skip_deps, skip_assets, fetch_assets, assets_manifest, yes):
    """Installs dependencies needed for your system (Linux/macOS/Windows)."""
    import os
    import platform
    import shutil
    import subprocess
...
""",
    ...
    if system == "Windows":
        setup_label = "setup.ps1"
        setup_script = os.path.join(script_dir, setup_label)
        ...
        if shell:
            ...
            if check_only:
                setup_cmd.append("-CheckOnly")
            if skip_system_deps:
                setup_cmd.append("-SkipSystemDeps")
            if skip_deps:
                setup_cmd.append("-SkipDeps")
            if skip_assets:
                setup_cmd.append("-SkipAssets")
            if fetch_assets:
                setup_cmd.append("-FetchAssets")
            if assets_manifest:
                setup_cmd.extend(["-AssetsManifest", assets_manifest])
            if yes:
                setup_cmd.append("-Yes")
        else:
            click.echo(
                click.style(
                    "❌ Error: PowerShell executable (pwsh/powershell) not found on PATH.",
                    fg="red",
                )
            )
    else:
        setup_label = "setup.sh"
        setup_script = os.path.join(script_dir, setup_label)
        bash = shutil.which("bash")
        if bash:
            setup_cmd = [bash, setup_script]
            if check_only:
                setup_cmd.append("--check-only")
            if skip_system_deps:
                setup_cmd.append("--skip-system-deps")
            if skip_deps:
                setup_cmd.append("--skip-deps")
            if skip_assets:
                setup_cmd.append("--skip-assets")
            if fetch_assets:
                setup_cmd.append("--fetch-assets")
            if assets_manifest:
                setup_cmd.extend(["--assets-manifest", assets_manifest])
            if yes:
                setup_cmd.append("--yes")
        else:
            click.echo(click.style("❌ Error: bash not found on PATH.", fg="red"))

    install_error = False
    if setup_cmd and os.path.exists(setup_script):
        click.echo(click.style("🚀 Setup initiated...\n", fg="green"))
        try:
            subprocess.check_call(setup_cmd, cwd=script_dir)
        ...
        click.echo(
            click.style(
                f"❌ Error: unable to run {setup_label or 'setup script'} (missing file or interpreter).",
                fg="red",
            )
        )
        install_error = True
...
@click.pass_context
def deploy(ctx):
    """One-click setup + start (wrapper around deploy.sh / deploy.ps1)."""
    import os
    import platform
    import shutil
    import subprocess
    ...
    if system == "Windows":
        deploy_script = os.path.join(repo_root, "deploy.ps1")
        shell = shutil.which("pwsh") or shutil.which("powershell")
        if not shell:
            click.echo(click.style("Error: PowerShell not found on PATH.", fg="red"))
            raise SystemExit(1)
        cmd = [
            shell,
            "-NoProfile",
        ...
        if not bash:
            click.echo(click.style("Error: bash not found on PATH.", fg="red"))
            raise SystemExit(1)
        cmd = [bash, deploy_script, *ctx.args]

    if not os.path.exists(deploy_script):
        click.echo(
            click.style(
                f"Error: deployment script not found: {deploy_script}",
                fg="red",
            )
        )
        raise SystemExit(1)
